    ...
    getProm() {
        ...
        for (const [k, v] of Object.entries(c)) {
            const name = k
                ...
                .toLowerCase();
            if (k === 'requests.total')
                lines.push(`requests_total ${v}`);
            else if (k === 'responses.total')
                lines.push(`responses_total ${v}`);
            else
                lines.push(`${name} ${v}`);
        }
        return lines.join('\n') + '\n';
...
], PrometheusController.prototype, "getProm", null);
exports.PrometheusController = PrometheusController = __decorate([
    (0, common_1.Controller)('metrics/prom'),
